...
class DDPG(object):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.actor = Actor(state_dim, action_dim, self.hidden_width, max_action).cuda()
        self.actor.apply(weight_init)
        self.actor_target = copy.deepcopy(self.actor).cuda()
        self.critic = Critic(state_dim, action_dim, self.hidden_width).cuda()
        self.critic.apply(weight_init)
        self.critic_target = copy.deepcopy(self.critic).cuda()
        ...
        while not done:
            ...
            if total_steps >= random_steps and total_steps % update_freq == 0:
                for _ in range(update_freq):
                    aloss, bloss = agent.learn(replay_buffer)
                    # print(f"round:{total_steps}, actor loss:{aloss}, critic loss:{bloss}")
            ...
            if (total_steps + 1) % evaluate_freq == 0:
                ...
                # print(f'step_rewards_{env_name[env_index]}_{total_steps}_{evaluate_reward}')
                # Save the rewards
                if evaluate_num % 10 == 0:
                    np.save('./logs/DDPG_env_{}_number_{}_seed_{}.npy'.format(env_name[env_index], number, seed),
